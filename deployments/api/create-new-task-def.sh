...
echo "Updating the container definition for container: $CONTAINER_NAME with new image: $NEW_IMAGE"
UPDATED_CONTAINER_DEFINITIONS=$(echo "$CONTAINER_DEFINITIONS" | jq -c --arg cn "$CONTAINER_NAME" --arg ni "$NEW_IMAGE" '
